...
function AppContent() {
  ...
  // Use real data hooks
  const { connectedSourcesCount: realTimeConnected, dataQuality: realTimeQuality } = useRealTimeData();
  const { 
  ...
  useEffect(() => {
    ...
    if (totalConnected > 0) {
      addToast(`🔴 Real Data Platform Active! Connected to ${totalConnected} live sources`, 'success');
    } else {
      addToast('⚠️ No real data sources available. Check API keys and network connection.', 'warning');
    }

    // API validation toast
    if (!validation.isValid) {
      addToast(`🔑 ${validation.missingKeys.length} API keys missing. Check Data Debug panel.`, 'warning');
    }
  }, [addToast, totalConnected, validation.isValid, validation.missingKeys.length]);
  ...
  return (
    <div className="flex h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-950 text-gray-900 dark:text-gray-100">
      ...
      <div className="flex-1 overflow-auto">
        <Header connectedSources={totalConnected} dataQuality={combinedQuality} />
        <div className="p-6">
