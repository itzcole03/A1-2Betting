...
import { Wifi, WifiOff, Database, TrendingUp, Users, Calendar } from 'lucide-react';
      ...
      <div className="relative z-10">
        <div className="text-center mb-8">
          ...
          <p className="text-xl max-w-4xl mx-auto leading-relaxed opacity-90">
            Advanced sports intelligence with <span className="font-bold text-yellow-300">{totalSources}+ real data sources</span>, 
            <span className="font-bold text-green-300"> PrizePicks integration</span>, 
            <span className="font-bold text-blue-300"> live market analysis</span>, and 
            <span className="font-bold text-red-300"> production-grade AI models</span> across all major sports
          </p>
          <div className="mt-4 flex items-center justify-center space-x-6 text-sm">
            <div className="flex items-center space-x-2">
              ...
              <span className={`font-medium ${isFullyConnected ? 'text-green-300' : 'text-yellow-300'}`}>
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="text-purple-300 font-medium">47+ AI Models Active</span>
            </div>
            <div className="text-gray-300">•</div>
            <div className="flex items-center space-x-2">
              <TrendingUp className="w-4 h-4 text-blue-400" />
              <span className="text-blue-300 font-medium">Real-Time Analysis</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-6 gap-4 mb-6">
          <div className="text-center p-4 glass-morphism rounded-xl">
            <div className="text-2xl font-bold text-green-400">{connectedSources}</div>
            <div className="text-xs opacity-80">Data Sources</div>
            <div className="text-xs text-green-300 mt-1">{connectionPercentage.toFixed(0)}% Active</div>
          </div>
          
          <div className="text-center p-4 glass-morphism rounded-xl">
            <div className="text-2xl font-bold text-yellow-400">{gamesCount}</div>
            <div className="text-xs opacity-80">Live Games</div>
            <div className="text-xs text-yellow-300 mt-1">All Sports</div>
          </div>
          
          <div className="text-center p-4 glass-morphism rounded-xl">
            <div className="text-2xl font-bold text-blue-400">{playersCount}</div>
            <div className="text-xs opacity-80">Active Players</div>
            <div className="text-xs text-blue-300 mt-1">Enhanced Data</div>
          </div>
          
          <div className="text-center p-4 glass-morphism rounded-xl">
            <div className="text-2xl font-bold text-purple-400">{(dataQuality * 100).toFixed(1)}%</div>
            <div className="text-xs opacity-80">Data Quality</div>
            <div className="text-xs text-purple-300 mt-1">Verified</div>
          </div>
          
          <div className="text-center p-4 glass-morphism rounded-xl">
            <div className="text-2xl font-bold text-indigo-400">{(dataReliability * 100).toFixed(1)}%</div>
        ...
        <div className="bg-gradient-to-r from-green-500/20 to-blue-500/20 rounded-2xl p-6 border border-white/20">
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              <div className={`text-2xl font-bold ${isPrizePicksConnected ? 'text-green-400' : 'text-yellow-400'}`}>
