...
export function RealPlayersDisplay({ players }: RealPlayersDisplayProps) {
  ...
  const getMainStat = (player: ProcessedPlayer) => {
    if (player.sport === 'NBA') {
      return { label: 'PPG', value: player.stats.points?.toFixed(1) || '0.0' };
    }
    if (player.sport === 'NHL') {
      return { label: 'Goals', value: player.stats.goals?.toFixed(1) || '0.0' };
    }
    if (player.sport === 'MLB') {
      return { label: 'AVG', value: player.stats.average?.toFixed(3) || '.000' };
    }
    return { label: 'Rating', value: '0.0' };
  };

  if (players.length === 0) {
    return (
      <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg">
        <h3 className="text-xl font-bold mb-4 dark:text-white">Real Player Data</h3>
        <div className="text-center py-8">
          <User className="w-12 h-12 text-gray-400 mx-auto mb-4" />
          <p className="text-gray-500 dark:text-gray-400">No player data available from real sources</p>
        </div>
      </div>
    );
  }
            ...
            >
              ...
              <div className="flex items-center justify-between text-sm">
                ...
                <div className="flex items-center space-x-1">
                  ...
                  <span className="text-gray-600 dark:text-gray-400">{player.source}</span>
                </div>
              </div>
